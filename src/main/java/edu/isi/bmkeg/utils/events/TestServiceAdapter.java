...
/**
 * Test service adapter. Great for testing when you want to JUST SEND AN OBJECT
 * and nothing else. This class has to stay in the main codebase (instead of
 * test) because, when it's used it needs to be deployed to Tomcat.
 * 
 * @author Kevin G [see http://stackoverflow.com/questions/1872742/
 *         how-to-push-data
 *         -from-blazeds-without-recieve-message-from-flex-client]
 * 
 */
public class TestServiceAdapter extends ServiceAdapter {
	...
	private Message createTestMessage() {
		...
		msg.setDestination("BmkegPushDestination");
	...
	private void secondsToSleep(int seconds) {
		try {
			...
			System.out
					.println("TestServiceAdapter Interrupted while sending messages");
			e.printStackTrace();
		}
	}
